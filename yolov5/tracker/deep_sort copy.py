...
from .deep.model import (get_model_link, is_model_in_factory,
...
class DeepSort(object):
    ...
    def __init__(self, model, device, max_dist=0.2,
                 max_iou_distance=0.7, max_age=70,
                 n_init=3, nn_budget=100):

        if is_model_in_factory(model):
            # download the model
            model_path = join('tracker/deep/checkpoint', model + '.pth')
            ...
            if is_model_type_in_model_path(model):
                model_path = join('tracker/deep/checkpoint', model)
                ...
                print(
                    'Cannot infere model name from provided DeepSort path, should be one of the following:')
        ...
        self.tracker = Tracker(
            metric, max_iou_distance=max_iou_distance,
